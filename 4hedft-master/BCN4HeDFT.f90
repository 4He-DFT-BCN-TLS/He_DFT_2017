...
If(limp)Then
     ...
     aux = sum(denx)*dxyz
     Write(6,'(" Normalitzacio de la impure�a...:",1p,E15.6)')aux
...
Endif
  ...
  If(lbulk)Then
    n4real=sum(den)*dxyz
    n4=n4+0.5
    Write(6,'(" Nombre de part�cules...:",1p,E15.6)')n4real
  Else 
    Write(6,'(" Nombre de part�cules...:",1p,E15.6)')(sum(den)*dxyz)
  Endif
...
!
!  Construim la matriu factt, responsable de la variaci� local del temps imaginary 
!
...
!
!  Per calcular amb la funcional s�lida s'ha de treure el terme amb Alpha_s
!
...
!   end if
   ...
   If(Trim(selec_gs).Eq.'LJ_OT')Then
      Write(6,"('Usamos la interacci�n de OT para la impuerza')")
   Endif        
   Do iz=1, nz
     Do iy=1, ny
       Do ix=1, nx/2 + 1
...
!
!  Despla�em la impure�a per tal que vagi cap el minim de potencial
!
...
!
!  Desplacem la impure�a si ho creiem convenient
!
   If(Mod(iter,pdespl).Eq.0.And.limp_despl.And..Not.limp)Then
     ...
     Do it=1,N_Newton
       ...
       If(N_Newton.Eq.it.Or.Dabs(aux).lt.prec_Newton)Then
         ...
         Endif
         Write(6,'("Despla�ament de la impure�a i nova posici�:",1p,2E18.10)')aux,zimp
       Endif
