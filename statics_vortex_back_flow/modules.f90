...
Logical                          :: Lvortex_dipole=.false., LOrtogonal=.false.
Logical                          :: Lvortex_ring=.false.
Logical                          :: Lsoliton=.false.
logical                          :: Lsymmetry=.false.    ! Flag to impose symmtry on the plane perpendicular to the vortex 
character (len =1)               :: Soliton_Axis='Z'     ! Could be: Z, Y or X
character (len =1)               :: Vortex_Axis='Z'      ! Could be: Z, Y or X
character (len =1)               :: Vortex_Ring_Axis='Z' ! Could be: Z, Y or X
real      (Kind=8)               :: C_symmetry=1.d3     ! Constraint to impose symmetry
...
real      (Kind=8)               :: rv=5.d0             ! Radius of a vortex ring
end module vortex
module alphasterm
...
Complex  (kind=8), allocatable :: pot4(:,:,:)     ! 'Potential' of lagrange equation
...
logical                       :: L_cilindrical_average=.false.  ! Flag to impose scilindrical average for the density
integer (kind=4), allocatable :: normr(:,:)                     ! Auxiliar vector to perform the cilindrical average
real    (kind=8), allocatable :: den_c_a(:,:,:)                 ! Cilindrical average density
real    (kind=8), allocatable :: denr(:,:)                      ! Cilindrical density
real    (kind=8), allocatable :: rr(:), Adenr(:,:), Bdenr(:,:)  ! Cilindrical radius, matrices to perform a bicubic spline
real    (kind=8)              :: hr=0.15d0        ! Cilindrical mesh
integer (kind=4)              :: nr               ! Number of points for the cilindrical average
end module field
!------------------------------------------------------------------
module fftmodule
...
integer (kind=8)              :: pfftfw_denx ! Pointer for FFT forward
...
integer (kind=8)              :: pfftfw_4    ! Pointer for FFT forward
integer (kind=8)              :: pfftbk_cg   ! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_potx4! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_upotx! Pointer for FFT bakward
...
integer (kind=8)              :: pfftbk_2    ! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_3    ! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_4    ! Pointer for FFT bakward
...
real      (kind=8)               ::      g= 0.d0            ! Exponent that modifies the cp4 & cpp4 terms  
...
logical                          :: L_modified_OT=.false.    ! Etiqueta para modificar las potencias de los terminos de correlacion
!
end module he4
!------------------------------------------------------------------
module impur
...
complex   (kind=8),allocatable   :: fdenx(:,:,:) ! For fourier transform of denx
...
complex (kind=8), allocatable :: Psi_gs(:,:,:)      ! Psi=sqrt(density) in real space
complex (kind=8), allocatable :: Psi_gsnw(:,:,:)    ! New value of Psi (temporal)
complex (kind=8), allocatable :: Psi_gs1(:,:,:)     ! Old Psi 1-step  backward
complex (kind=8), allocatable :: Psi_gs2(:,:,:)     ! Old Psi 1-step  backward
...
module backflow
!complex (kind=8), allocatable :: dxpsi(:,:,:)      ! Partial derivative in X for Psiv in Real-space
!complex (kind=8), allocatable :: dypsi(:,:,:)      ! Partial derivative in X for Psiv in Real-space
!complex (kind=8), allocatable :: dzpsi(:,:,:)      ! Partial derivative in X for Psiv in Real-space
real    (kind=8), allocatable :: vel(:,:,:,:)      ! Velocity field
real    (kind=8), allocatable ::   ulj(:,:,:)      ! Analityc Fourier transform pf backflow term
complex (kind=8), allocatable :: pot_bf(:,:,:)     ! Backflow potential term
real    (kind=8), allocatable :: pot_bf_aux(:,:,:) ! Backflow potential term
real    (kind=8), allocatable :: penalty_bf(:,:,:) ! Penalty function for the backflow potential term
real    (kind=8)              :: e_bf              ! Back flow energy contribution
real    (kind=8)              :: Min_rho=1.d-4     ! Cutoff for low  densities.
real    (kind=8)              :: Max_rho=3.5d-2   ! Cutoff for high densities.
logical                       :: lbackflow=.false. ! Use back flow terms or not
end module backflow
!------------------------------------------------------------------
