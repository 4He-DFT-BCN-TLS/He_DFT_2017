...
integer    (kind=4)  :: ix ,iy ,iz, ir      ! Used as indexs of arrays
...
real       (kind=8)  :: denxmax,xcm,ycm,zcm         ! Maximum value for density of the impurity
...
real       (kind=8)  :: old_L=0.        ! Auxiliar variables
real       (kind=8)  :: eold,dife       ! Auxiliar variables
real       (kind=8)  :: eold_new        ! Auxiliar variables
complex    (kind=8)  :: ci=(0.d0,1.d0)  ! Imaginary unit
complex    (kind=8)  :: caux            ! Auxiliar variables
real       (kind=8)  :: aux,aux1,aux2   ! Auxiliar variables
...
namelist /vortex_ring_z/rv,zv    ! zv is the palen that will contain the vortex ring
namelist /vortex_ring_y/rv,yv    ! yv is the palen that will contain the vortex ring
namelist /vortex_ring_x/rv,xv    ! xv is the palen that will contain the vortex ring
namelist /input/title,fftwplan,nthread,nsfiles,                         &
                ...
                L_anell, r_anell, a_anell,Lsymmetry,c_symmetry,             &
                L_esfera, r_esfera, a_esfera,Lvortex_dipole,                &
                LOrtogonal,Lvortex_ring,Vortex_ring_axis,LSoliton,          &
                Soliton_axis,L_modified_OT,g,L_cilindrical_average,hr,      &
                Lbackflow,Min_rho,Max_rho
!................................ Start main Program ..............................
call timer(t0)
...
write(10,nml=input)
IF(Lvortex.And.nv.Eq.1.And.omega.Ne.0.d0)Then
  Write(6,'(" For nv=1, omega must be 0;....:",1p,E15.6)')omega      
  Stop '001'
EndiF
IF(Lvortex)then
  If(Lvortex_dipole)Then
    If(nv.Ne.2)Then
      Write(6,'(" For Lvortex_dipole, nv must be 2....",I5)')nv
      Stop '002'
    EndIf  
    If(omega.Ne.0.d0)Then
      Write(6,'(" For Lvortex_dipole, omega must be 0....",1p,E15.6)')omega
      Stop '003'
    EndIf  
  EndIf  
  IF(Lvortex_ring)Then
    If(nv.Ne.1)Then
      Write(6,'(" For Lvortex_ring, nv must be 1....",I5)')nv
      Stop '004'
    EndIf  
    If(omega.Ne.0.d0)Then
      Write(6,'(" For Lvortex_ring, omega must be 0....",1p,E15.6)')omega
      Stop '005'
    EndIf  
  EndIf  
Endif
If(mode.Ne.0.And.mode.Ne.4)Then
  IF(Lvortex)Then
    IF(.Not.Lvortex_ring)Then
      ...
      EndIf
    Else
      If(Vortex_Ring_Axis.Eq.'Z')Then
        Allocate(zv(nv))
        Open(unit=1,File='vortex.input')    
        Read(1,vortex_ring_z)
        Write(6,vortex_ring_z)
        Close(Unit=1)
      EndIf  
      If(Vortex_Ring_Axis.Eq.'Y')Then
        Allocate(yv(nv))
        Open(unit=1,File='vortex.input')    
        Read(1,vortex_ring_y)
        Write(6,vortex_ring_y)
        Close(Unit=1)
      EndIf  
      If(Vortex_Ring_Axis.Eq.'X')Then
        Allocate(xv(nv))
        Open(unit=1,File='vortex.input')    
        Read(1,vortex_ring_x)
        Write(6,vortex_ring_x)
        Close(Unit=1)
      EndIf
    EndIf
  EndIf        
...
If(.Not.Lvortex.Or.nv.Gt.1.Or.Lvortex_ring.Or.Lvortex_dipole)Then
  If(L_cilindrical_average)Then
    Write(6,'("There is no sense to perform a cilindrical avergae")')
    Stop  "001"
  EndiF    
Endif
...
If(L_cilindrical_average)Then
  Allocate(den_c_a(nx,ny,nz))
  If(Vortex_axis.Eq.'Z')Then      
    rmax=Dsqrt((xmax-xc)**2 + (ymax-yc)**2) 
    nr = (rmax/hr + 1.5)
    Write(6,'("Cilindrical average (around Z) information: hr, nr...",1pE15.6,0p,I10)')hr, nr
    Allocate(rr(nr))
    Allocate(normr(nr,nz))
    Allocate(denr(nr,nz))
    Allocate (Adenr(nr,nz))
    Allocate (Bdenr(nr,nz))
    Do ir = 1, nr
      rr(ir) = (ir-1)*hr
    EndDo
  ElseIf(Vortex_axis.Eq.'Y')Then  
    rmax=Dsqrt((xmax-xc)**2 + (zmax-zc)**2) 
    nr = (rmax/hr + 1.5)
    Write(6,'("Cilindrical average (around Y) information: hr, nr...",1pE15.6,0p,I10)')hr, nr
    Allocate(rr(nr))
    Allocate(normr(nr,ny))
    Allocate(denr(nr,ny))
    Allocate (Adenr(nr,ny))
    Allocate (Bdenr(nr,ny))
    Do ir = 1, nr
      rr(ir) = (ir-1)*hr
    EndDo
  Else  
    rmax=Dsqrt((ymax-yc)**2 + (zmax-zc)**2) 
    nr = (rmax/hr + 1.5)
    Write(6,'("Cilindrical average (around X) information: hr, nr...",1pE15.6,0p,I10)')hr, nr
    Allocate(rr(nr))
    Allocate(normr(nr,nx))
    Allocate(denr(nr,nx))
    Allocate (Adenr(nr,nx))
    Allocate (Bdenr(nr,nx))
    Do ir = 1, nr
      rr(ir) = (ir-1)*hr
    EndDo
  EndIf
...
If(mode.Ne.0.And.mode.Ne.4)Then
  If(Lvortex)Then 
    If(Lvortex_dipole)Then
!            
!  We imprint a vortex dipole
!            
      If(Vortex_axis.Eq.'Z')Then      
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = DSqrt((x(ix)-xv(1))**2+(y(iy)-yv(1))**2)
              If(aux1.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((x(ix)-xv(1)),(y(iy)-yv(1)))/aux1
              Endif
              aux2 = DSqrt((x(ix)-xv(2))**2+(y(iy)-yv(2))**2)
              If(aux2.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Caux * Cmplx((x(ix)-xv(2)),-(y(iy)-yv(2)))/aux2
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                psi_gs(ix,iy,iz) = Dexp(-aux1) + Dexp(-aux2)     
              EndIf                
            EndDo
          EndDo
        EndDo
      ElseIf(Vortex_axis.Eq.'Y')Then      
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = DSqrt((x(ix)-xv(1)**2+(z(iz)-zv(1))**2))
              If(aux1.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((x(ix)-xv(1)),(z(iz)-zv(1)))/aux1
              Endif
              aux2 = DSqrt((x(ix)-xv(2)**2+(z(iz)-zv(2))**2))
              If(aux2.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Caux*Cmplx((x(ix)-xv(2)),-(z(iz)-zv(2)))/aux2
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                psi_gs(ix,iy,iz) = Dexp(-aux1) + Dexp(-aux2)     
      ...
      ElseIf(Vortex_axis.Eq.'X')Then
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = DSqrt((y(iy)-yv(1))**2+(z(iz)-zv(1))**2)
              If(aux1.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((y(iy)-yv(1)),(z(iz)-zv(1)))/aux1
              Endif
              aux2 = DSqrt((y(iy)-yv(2))**2+(z(iz)-zv(2))**2)
              If(aux2.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Caux*Cmplx((y(iy)-yv(2)),-(z(iz)-zv(2)))/aux2
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                psi_gs(ix,iy,iz) = Dexp(-aux1) + Dexp(-aux2)     
        ...
        EndDo
      Else
        Write(6,'("Vortex_axis must be: X, Y or Z....",A)')Vortex_axis     
        Stop '001'
      EndIf        
!            
!  We imprint a vortex ring
!            
    ElseIf(Lvortex_ring)Then
      If(Vortex_Ring_Axis.Eq.'Z')Then
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = Sqrt(x(ix)**2+y(iy)**2)  
              aux = DSqrt((z(iz)-zv(1))**2 + (aux1 - rv)**2)
              If(aux.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((z(iz)-zv(1)),(aux1-rv))/aux
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                aux = (aux/afermi)**2
                psi_gs(ix,iy,iz) = Dexp(-aux)     
              EndIf                
            EndDo
          EndDo
        EndDo
      ElseIf(Vortex_Ring_Axis.Eq.'Y')Then
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = Sqrt(x(ix)**2+z(iz)**2)  
              aux = DSqrt((y(iy)-yv(1))**2 + (aux1 - rv)**2)
              If(aux.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((y(iy)-yv(1)),(aux1-rv))/aux
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                 aux = (aux/afermi)**2
          ...
          EndDo
        EndDo
      ElseIf(Vortex_Ring_Axis.Eq.'X')Then
        Do iz = 1, nz
          Do iy = 1, ny
            Do ix = 1, nx
              aux1 = Sqrt(y(iy)**2+z(iz)**2)  
              aux = DSqrt((x(ix)-xv(1))**2 + (aux1 - rv)**2)
              If(aux.eq.0.0d0)then
                Caux = (0.d0, 0.d0)
              Else
                Caux = Cmplx((x(ix)-xv(1)),(aux1-rv))/aux
              Endif
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))*Caux       
              If(LOrtogonal)Then
                aux = (aux/afermi)**2
        ...
        EndDo
      Else        
        Write(6,'("Vortex_Ring_Axis must be: X, Y or Z....",A)')Vortex_ring_axis     
        Stop '002'
      EndIF        
    Else
!            
!  We imprint a normal nv vortex
!            
      If(Vortex_axis.Eq.'Z')Then
        Do iz = 1, nz
          Do iy = 1, ny
      ...
      Endif
    EndIf        
  ElseIf(Lsoliton)Then
!            
!  We imprint a soliton
!            
    If(Soliton_Axis.Eq.'Z')Then
      Do iz = 1, nz
        Do iy = 1, ny
          Do ix = 1, nx
            If(iz.Lt.(nz/2+1))Then
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))    
            ElseIf(iz.Eq.(nz/2+1))Then
              Psi(ix,iy,iz) = 0.d0    
            Else
              Psi(ix,iy,iz) = - Sqrt(den(ix,iy,iz))    
            EnDif  
        ...
        EndDo
      EndDo
    ElseIf(Soliton_Axis.Eq.'Y')Then
      Do iz = 1, nz
        Do iy = 1, ny
          Do ix = 1, nx
            If(iy.Lt.(ny/2+1))Then
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))    
            ElseIf(iy.Eq.(ny/2+1))Then
              Psi(ix,iy,iz) = 0.d0    
            Else
              Psi(ix,iy,iz) = - Sqrt(den(ix,iy,iz))    
            EnDif  
        ...
        EndDo
      EndDo
    ElseIf(Soliton_Axis.Eq.'X')Then
      Do iz = 1, nz
        Do iy = 1, ny
          Do ix = 1, nx
            If(ix.Lt.(nx/2+1))Then
              Psi(ix,iy,iz) = Sqrt(den(ix,iy,iz))    
            ElseIf(ix.Eq.(nx/2+1))Then
              Psi(ix,iy,iz) = 0.d0    
            Else
              Psi(ix,iy,iz) = - Sqrt(den(ix,iy,iz))    
            EnDif  
      ...
      EndDo
    Else        
      Write(6,'("Soliton_Axis must be: X, Y or Z....",A)')Soliton_axis     
      Stop '008'
    EndIF        
    If(LOrtogonal)Then
      aux = Sqrt(1.d0/Sum(Conjg(psi_gs)*psi_gs)*hx*hy*hz)
      psi_gs = psi_gs*aux     
      psi_gs1 = psi_gs
      psi_gs2 = psi_gs
    EndIf                
  Else
    Write(6,'("We do not imprint any thing....")')  
  Endif
  den = Conjg(Psi)*Psi
Endif
!===============================================================================
...
!..............................................................................
     ...
     If(L_cilindrical_average)Then
       ...
       den_c_a = den              ! We store the cilindrical avergae density
       den = Abs(psi)**2          ! We recovered the input density (not averaged)
       If(Vortex_axis.Eq.'Z')Then
         aux = 0.d0
         Do iz=1, nz
           aux1 = 0.d0
           Do ir=1, nr
             aux1 = aux1 + rr(ir)*denr(ir,iz)
           EndDo  
           aux = aux +aux1
         EndDo  
         aux = aux*2.d0*Pi*hr*hz
         Write(6,'("n4 & Integration of the cilindrical average....",I5,1pE15.6)')n4,aux
         Open(Unit=1,file='den0-r.dat')
         Write(1,'("# r, den(r,z=0)")')
         Do ir=1,nr
           Write(1,'(1p,E15.6,5E19.11)')rr(ir),denr(ir,(nz/2+1))
         Enddo
         Close(Unit=1)
       ElseIf(Vortex_axis.Eq.'Y')Then
         aux = 0.d0
         Do iy=1, ny
           aux1 = 0.d0
           Do ir=1, nr
             aux1 = aux1 + rr(ir)*denr(ir,iy)
           EndDo  
           aux = aux +aux1
         EndDo  
         aux = aux*2.d0*Pi*hr*hy
         Write(6,'("n4 & Integration of the cilindrical average....",I5,1pE15.6)')n4,aux
         Open(Unit=1,file='den0-r.dat')
         Write(1,'("# r, den(r,y=0)")')
         Do ir=1,nr
           Write(1,'(1p,E15.6,5E19.11)')rr(ir),denr(ir,(ny/2+1))
         Enddo
         Close(Unit=1)
       Else        
         aux = 0.d0
         Do ix=1, nx
           aux1 = 0.d0
           Do ir=1, nr
             aux1 = aux1 + rr(ir)*denr(ir,ix)
           EndDo  
           aux = aux +aux1
         EndDo  
         aux = aux*2.d0*Pi*hr*hx
         Write(6,'("n4 & Integration of the cilindrical average....",I5,1pE15.6)')n4,aux
         Open(Unit=1,file='den0-r.dat')
         Write(1,'("# r, den(r,x=0)")')
         Do ir=1,nr
           Write(1,'(1p,E15.6,5E19.11)')rr(ir),denr(ir,(nx/2+1))
         Enddo
         Close(Unit=1)
       EndIf        
     EndIf        
...
if(limp) then
   ...
   write(6,6138) h2o2mx
   write(6,*) ' '
   write(6,*) '    Calculation with the impurity: ',selec
...
If(Lsolid) Then
  core4='OT '
  Write(6,'(" We will use the solid DF + OT without alphas term")') 
Endif  
If(L_modified_OT)Then
  Write(6,'(" We will use the modified OT DF, see D. Mateo et al. JCP 138, 044321 (2013) pag 3")')
Else
  Write(6,'(" We will use the standard OT DF")')
EndIf        
...
select case(core4)
   ...
   case('OTE')
     ...
     If(.Not.Allocated(intxalf))allocate(intxalf(nx  ,ny,nz))
     If(.Not.Allocated(intyalf))allocate(intyalf(nx  ,ny,nz))
     If(.Not.Allocated(intzalf))allocate(intzalf(nx  ,ny,nz))
   case('OTC')
     h4=h4ot
     write(6,*) '    Use Orsay-Trento Interaction.'
...
If(Lbackflow)Call vforma(nx,ny,nz,pmod,ulj)
...
!   umax=umax_gs
!   call potimpx()
    psi1  = psi      ! Array initialized for imaginary step-time
    psi2  = psi      ! Array initialized for imaginary step-time
    If(L_cilindrical_average)Then
      den = den_c_a               ! We recover the cilindrical avergae density
    EndIf  
!   call fftfw(den ,fden)
...
if(Lbackflow)write(6,'(T5,"Backflow energy...............: ",1p,3x,E15.8," K")')e_bf
...
!                (y(iy)*sto1c(ix,iy,iz) - x(ix)*sto2c(ix,iy,iz)) 
          ...
          EndDo
          caux = caux*dxyz
          Write(6,'(T5,"<L_z>.........................:",8x,1p,E15.6," \hbar +-",E13.4," \hbar")')Real(caux), (Real(caux)-old_L)
          old_L = Real(caux)
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_z>..................:",8x,1p,E15.6," K")')-Real(omega*caux) 
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_z>........:",3x,1p,E20.11," K")')etot 
          Endif
        Endif
        If(Vortex_axis.Eq.'Y')Then
          Call derivnD(1,nn,hx,1,psi,sto1c,Icon)
...
!                (x(ix)*sto2c(ix,iy,iz) - z(iz)*sto1c(ix,iy,iz)) 
                ((x(ix)-xcm)*sto2c(ix,iy,iz) - (z(iz)-zcm)*sto1c(ix,iy,iz)) 
              EndDo
            EndDo
          EndDo
          caux = caux*dxyz
          Write(6,'(T5,"<L_y>.........................:",8x,1p,E15.6," \hbar +-",E13.4," \hbar")')Real(caux), (Real(caux)-old_L)
          old_L = Real(caux)
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_y>..................:",8x,1p,E15.6," K")')-Real(omega*caux)
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_y>........:",3x,1p,E20.11," K")')etot 
          Endif
        Endif
        If(Vortex_axis.Eq.'X')Then
...
!                (z(iz)*sto1c(ix,iy,iz) - y(iy)*sto2c(ix,iy,iz)) 
                ((z(iz)-zcm)*sto1c(ix,iy,iz) - (y(iy)-ycm)*sto2c(ix,iy,iz)) 
              EndDo
            EndDo
          EndDo
          caux = caux*dxyz
          Write(6,'(T5,"<L_x>.........................:",8x,1p,E15.6," \hbar +-",E13.4," \hbar")')Real(caux), (Real(caux)-old_L)
          old_L = Real(caux)
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_x>..................:",8x,1p,E15.6," K")')-Real(omega*caux)
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_x>........:",3x,1p,E20.11," K")')etot 
          Endif
        Endif
        If(Lsymmetry.And.nv.Gt.1)Then
          Write(6,'(T5,"C_symmetry*(xcm**2+ycm**2+zcm**2)..:",8x,1p,E15.6," K")')(-0.5d0*C_symmetry*(xcm**2+ycm**2+zcm**2))
        EndIf   
        If(nv.Gt.1)then
          eold_new=etot
          etot = etot + omega*caux
...
if(limp) Then
  psi1x = psix
  psi2x = psix
  call evolox(0.0d0,epsx,epsxerr)        ! First autovalue for impurity
EndIf
...
!      call fftbk(wk1,dencg)                      ! Get Den of Coarse graining
      call fftbk_cg()                            ! Get Den of Coarse graining
      forall(ix=1:nx/2+1,iy=1:ny,iz=1:nz)
         wk1(ix,iy,iz) = fden(ix,iy,iz)*kalfs(ix,iy,iz)
      end forall
      ...
      If(Lvortex)Then
        ...
        If(Vortex_axis.Eq.'Z')Then
          ...
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_z>...............",8x,1p,E15.6," K")')-Real(omega*caux)
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_z>.....",3x,1p,E20.11," K +-",E15.6," K")')etot,(etot-eold_new)
          Endif
        Endif
        If(Vortex_axis.Eq.'Y')Then
          Call derivnD(1,nn,hx,1,psi,sto1c,Icon)
          ...
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_y>...............",8x,1p,E15.6," K")')-Real(omega*caux)
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_y>.....",3x,1p,E20.11," K +-",E15.6," K")')etot,(etot-eold_new)
          Endif
        Endif
        If(Vortex_axis.Eq.'X')Then
          Call derivnD(1,nn,hy,2,psi,sto2c,Icon)
          ...
          Do iz=1, nz
            Do iy=1, ny
              Do ix=1, nx
                caux = caux + Ci*Conjg(Psi(ix,iy,iz))*                  &
                ((z(iz)-zcm)*sto2c(ix,iy,iz) - (y(iy)-zcm)*sto3c(ix,iy,iz)) 
              EndDo
            EndDo
          EndDo
          caux = caux*dxyz
          ...
          If(nv.Gt.1)Then
            Write(6,'(T5,"-Omega*<L_x>...............",8x,1p,E15.6," K")')-Real(omega*caux)
            etot = etot -omega*caux
            Write(6,'(T5,"<He + X> - Omega*<L_x>.....",3x,1p,E20.11," K +-",E15.6," K")')etot,(etot-eold_new)
          Endif
        Endif
        If(Lsymmetry.And.nv.Gt.1)Then
          Write(6,'(T5,"C_symmetry*(xcm**2+ycm**2+zcm**2)..:",8x,1p,E15.6," K")')(-0.5d0*C_symmetry*(xcm**2+ycm**2+zcm**2))
...
!                    deltat/deltat0,iter)
     ...
     call printout(3,namefile,namefile1,deltat/deltat0,iter)
...
!..............................................................................
     ...
     Enddo
     Close(Unit=1)
     If(Lvortex_dipole.Or.Lvortex_Ring.Or.Lsoliton)Then
       If(LOrtogonal)Then      
         Open(Unit=1,file='psi-x.dat')
         Do ix=1,nx
           Write(1,'(1p,E15.6,5E19.11)')x(ix),psi(ix,ny/2+1,nz/2+1),psi_gs(ix,ny/2+1,nz/2+1)
         Enddo
         Close(Unit=1)

         Open(Unit=1,file='psi-y.dat')
         Do iy=1,ny
           Write(1,'(1p,E15.6,5E19.11)')y(iy),psi(nx/2+1,iy,nz/2+1),psi_gs(nx/2+1,iy,nz/2+1)
         Enddo
         Close(Unit=1)

         Open(Unit=1,file='psi-z.dat')
         Do iz=1,nz
           Write(1,'(1p,E15.6,5E19.11)')z(iz),psi(nx/2+1,ny/2+1,iz),psi_gs(nx/2+1,ny/2+1,iz)
         Enddo
         Close(Unit=1)
       Else  
         Open(Unit=1,file='psi-x.dat')
         Do ix=1,nx
           Write(1,'(1p,E15.6,5E19.11)')x(ix),psi(ix,ny/2+1,nz/2+1)
         Enddo
         Close(Unit=1)

         Open(Unit=1,file='psi-y.dat')
         Do iy=1,ny
           Write(1,'(1p,E15.6,5E19.11)')y(iy),psi(nx/2+1,iy,nz/2+1)
         Enddo
         Close(Unit=1)

         Open(Unit=1,file='psi-z.dat')
         Do iz=1,nz
           Write(1,'(1p,E15.6,5E19.11)')z(iz),psi(nx/2+1,ny/2+1,iz)
         Enddo
         Close(Unit=1)
       EndIf  
     EndIf
...
!     end if
     ...
     call printout(2,namefile,namefile1,deltat/deltat0,iter)
...
!end if
  ...
  call printout(1,filedenout,fileimpout,deltat/deltat0,iter)
