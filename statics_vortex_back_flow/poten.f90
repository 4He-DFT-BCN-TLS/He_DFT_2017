...
If(L_modified_OT)Then
  ...
  forall(ix=1:nx,iy=1:ny,iz=1:nz)
     sto1(ix,iy,iz) = den(ix,iy,iz)**(1.d0+g)*(a0*(2.d0-g)*dencg(ix,iy,iz)**(1.d0-g)+a1*(3.d0-g)*dencg(ix,iy,iz)**(2.d0-g))   ! g=0  is equivalent to the below expression
  end forall
Else        
  forall(ix=1:nx,iy=1:ny,iz=1:nz)
...
! The next expression must be equivalent to term below when g=0              
                       (1.d0+g)*den(ix,iy,iz)**g*(a0*dencg(ix,iy,iz)**(2.d0-g)+a1*dencg(ix,iy,iz)**(3.d0-g)) +      &   ! Correlation
...
if(limp) then
   ...
   call fftbk_upotx()    ! Get  ( int{ rho_4    * V_X dr'} ) Potential due to 4He for the impurity

   forall(ix=1:nx/2+1,iy=1:ny,iz=1:nz)
      wk1(ix,iy,iz) = fdenx(ix,iy,iz)*vq(ix,iy,iz)  ! FFT(den_x)*FFT(V_x)
   end forall 

   call fftbk_potx4()     ! Get  ( int{ Psi_x**2 * V_X dr'} ) Potential due to the impurity for the 4He
...
If(Lbackflow)Then
!  Write(6,'("Abans de trucar a poten_v...")')
!  Call flush(6)  
  call poten_v()      
!  Write(6,'("Despres de trucar a poten_v...")')
!  Call flush(6)  
  pot4 = pot4 +pot_bf
EndIf
If(lconstraint)Then
