...
do iz=1,nz
   do iy=1,ny
      do ix=1,nx
          aux3  = den(ix,iy,iz)
          aux4  = Abs(dencg(ix,iy,iz))
          If(L_modified_OT)Then
            aux5  = aux3**(1.d0+g)*(aux1*aux4**(2.d0-g)+aux2*aux4**(3.d0-g))    ! g=0 it means standard OT functional (see next line)
          Else        
