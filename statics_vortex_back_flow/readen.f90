...
real      (kind=8) :: vximpp,vyimpp,vzimpp
...
select case(mode)
!-------------------------------------------------------------------
  case(0)   ! Continue a calculation (read a previous wave function)
!-------------------------------------------------------------------
...
!
     ...
     If(LOrtogonal)Then
       read(1,*)psi_gs
       aux = 1.d0/(Sqrt(Sum(Conjg(psi_gs)*psi_gs))*hxp*hyp*hzp)
       psi_gs = psi_gs*aux
     EndIf  
     close(1)
...
!-------------------------------------------------------------------
     ...
     open(unit=1,file=fileden,status='old')
     call titols(1,cchar,isalto)
     read(1,*) xmaxp,ymaxp,zmaxp,hxp,hyp,hzp,nxp,nyp,nzp,limp, ximpp, yimpp, zimpp       ! We read the dummy position 
!
     psidum=(0.0d0, 0.0d0)         ! Be sure that all the components are initialized
!                                  ! to zero.
!
...
!
     ...
     If(LOrtogonal)Then
       read(1,*)dendum
!       psidum = Sqrt(dendum)*(1.d0,1.d0)
!       psidum(:,ny/2+1,:) = (0.d0, 0.d0)
!       do iy = ny/2+2, ny
!         psidum(:,iy,:) = psidum(:,iy,:)*(1.d0,-1.d0)
!       EndDo
!       aux = Sum(conjg(psidum)*psidum)*hxp*hyp*hzp
!       aux = Sqrt(npart/aux)
!       psidum = psidum*aux
!       aux = Sum(conjg(psidum)*psidum)*hxp*hyp*hzp
!       Write(6,'("From readen(1): Number of particles...:",1p,E15.8)')aux
       aux = Sum(dendum)*hxp*hyp*hzp
       Write(6,'("From readen(2): Number of particles...:",1p,E15.8)')aux
       aux = Sqrt(1.d0/npart)
       psi_gs = Sqrt(dendum)*aux
     EndIf  
