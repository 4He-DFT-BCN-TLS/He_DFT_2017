...
subroutine printout(iprint,namefile,namefile1,paflv,iter)

use rho       ! psi, psi_gs, den
use impur     ! psix, ximp, yimp, zimp, selec
use grid      ! xmax, ymax, zmax, hx, hy, hz, nx, ny, nz
use vortex    ! Lvortex_dipole, LOrtogonal
...
if(iprint.lt.0) then
   ...
   write(10) psi
   If(Lvortex_dipole.And.LOrtogonal)write(10) psi_gs
   close(10)
   if(limp) then
...
!
  ...
  if(limp) then
    write(10,1010) iter,paflv,selec
  else
...
!...(Print the density of helium)
  write(10,*) xmax,ymax,zmax,hx,hy,hz,nx,ny,nz,limp
  write(10,*) ximp,yimp,zimp        ! Impurity position, to be consistent with the dynamic program
  write(10,*) psi
  If(LOrtogonal)write(10,*) psi_gs
  close(10)
